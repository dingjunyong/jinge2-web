...
namespace Nop.Services.Common
{
    ...
    public partial class SearchTermService : ISearchTermService
    {
        ...
        public virtual IPagedList<SearchTerm> GetSearchTerms(int storeId = 1, bool isOrderByCountDesc = false, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _searchTermRepository.Table;

            if (isOrderByCountDesc)
            {
                query = query.OrderByDescending(order => order.Count);
            }
            else
            {
            ...
            var result = new PagedList<SearchTerm>(query, pageIndex, pageSize);
            return result;
        }
